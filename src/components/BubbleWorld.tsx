...
const BubbleWorld = ({ topics, onBubbleClick }: BubbleWorldProps) => {
  ...
  if (!isInitialized && !hasError) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-[#F9F7F0]/50">
        <div className="flex flex-col items-center">
          <div className="w-16 h-16 border-4 border-[#ebbd34] border-t-transparent rounded-full animate-spin"></div>
          <p className="mt-4 text-[#ebbd34] font-medium">Loading Bubble World...</p>
        </div>
      </div>
    );
  }
  ...
  if (hasError) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-[#F9F7F0]/50">
        <div className="text-center bg-white/80 p-6 rounded-lg shadow-md max-w-md">
          <div className="text-red-500 mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Unable to Load Bubble World</h3>
          <p className="text-gray-600 mb-4">We encountered a problem while loading the Bubble World visualization. Please refresh the page to try again.</p>
          <button 
            onClick={() => window.location.reload()} 
            className="px-4 py-2 bg-[#ebbd34] text-white rounded-md hover:bg-[#ebbd34]/90 transition-colors"
          >
            Refresh Page
          </button>
        </div>
